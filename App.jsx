...
function KPI({label,value,sub,color="#26a69a",size=24,th}) {
  ...
  const bg  = th?.card    || "#1e222d";
  const bdr = th?.border  || "rgba(255,255,255,0.06)";
  const dim = th?.textDim || "rgba(241,245,249,0.4)";
  return (
    <div className="kpi-card" style={{background:bg,border:"1px solid "+bdr,borderRadius:4,padding:"18px 20px",position:"relative",overflow:"hidden",transition:"all 0.2s ease"}}>
      <div style={{position:"absolute",top:0,left:0,right:0,height:2,background:`linear-gradient(90deg,${color}80,transparent)`}} />
      <div style={{fontSize:8,color:color+"99",textTransform:"uppercase",letterSpacing:2,fontFamily:"'IBM Plex Sans',sans-serif",fontWeight:700,marginBottom:8}}>{label}</div>
      <div style={{fontSize:size,fontWeight:800,color,fontFamily:"'IBM Plex Sans',sans-serif",letterSpacing:-0.5,lineHeight:1}}>{value}</div>
      {sub&&<div style={{fontSize:9,color:dim,marginTop:6,fontFamily:"'IBM Plex Sans',sans-serif"}}>{sub}</div>}
    </div>
  );
}
